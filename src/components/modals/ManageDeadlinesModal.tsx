...
export function ManageDeadlinesModal({ isOpen, onClose, hiveId, deadlines }: ManageDeadlinesModalProps) {
  ...
  function handleDelete(deadlineId: string, deadlineTitle: string) {
    startDeleteTransition(async () => {
      removeOptimisticDeadline(deadlineId);
      toast.promise(
        (async () => {
          const result = await deleteDeadline(hiveId, deadlineId);
          if (result && "error" in result && result.error) throw new Error(result.error);
        })(),
        {
          loading: `Removing "${deadlineTitle}"…`,
          success: `"${deadlineTitle}" removed.`,
